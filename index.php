<html>
<head>
    <title>Bibliotekos informacinė sistema</title>
</head>

<body>
    <a href="atsijungimas.php">Atsijungti</a><br/>
    <a href="paskyrosRedagavimas.php">Redaguoti paskyrą</a><br/>
    <center>
        <h1>Bibliotekos informacinė sistema</h1>
        <table border="1" cellpadding="10">
            <tr align="center">
                <td><a href="knyguSarasas.php">Knygų sąrašas</a></td>
                <td><a href="ataskaituSarasas.php">Ataskaitų sąrašas</a></td>
                <td>Darbuotojams:<br/><a href="Darbuotoju_modulis/skoluSarasas.php">Skolų sąrašas</a><br/>
                    Klientams:<br/><a href="Klientu_modulis/skolosLangas.php">Skolos langas</a></td>
                <td>Klientams:<br/><a href="Klientu_modulis/rezervacijuSarasas.php">Knygų rezervacijų sąrašas</a></td>
                <td>Darbuotojams:<br/><a href="Darbuotoju_modulis/sutarciuSarasas.php">Sutarčių sąrašas</a></td>
                <td>Darbuotojams:<br/><a href="Darbuotoju_modulis/darbuotojuSarasas.php">Darbuotojų sąrašas</a></td>
                <td><a href="renginiuKalendorius.php">Renginių kalendorius</a></td>
            </tr>
        </table>
    </center>
</body>

</html>